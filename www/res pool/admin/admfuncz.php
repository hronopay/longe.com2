...
Выберите ИП:
	<select name="userlogin"><?
 	while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		?><option value="<?=$row['userlogin'];?>"><?=$row['fio'];?></option><?
	}
?>
	</select>
...
Создать нового партнёра:<br>

	<select name="userloginfirst">
			<option value="2">ООО</option>
			<option value="1">ИП</option>
	</select><br>
	Название или ФИО <br><input type="text" name="fio" value="" size="40" title="Название или ФИО"><br>
	Пароль <br><input type="text" name="ph" value="" size="4" title="Пароль"><br>
	Телефон <br><input type="text" name="tele_concact" value="" size="40" title="Телефон"><br>

	Договор на Юрадрес <br>
	<input name="startdate" type="text" size="10" value="<?php echo date("20y-m-d");?>"  onclick="ds_sh(this);" readonly="readonly" style="cursor: text" title="кликнуть лев. кнопкой мыши">
...
if ($partner_type < 10) include('includes/anketa.php');
...
<thead>
<tr><td>N</td><td>Дата</td><td>переадр.</td><td>абонент</td><td>сек</td><td>мин</td><th>Доход</th><th>Расх.</th><th>Прибыль</th><th>Собрано</th></tr>
</thead>
<tbody>
<?
...
<tr>
  <th>N</th>
  <th colspan="4"><div align="right">Итого: Тарифицированная длительность  <? echo $total_dur; ?> сек</div></th>
  <td><? echo $total_mins; ?>м</td>
  <th><div align="right"><? echo $total_In; ?>p</div></th>
  <th><div align="right"><? echo $total_Out; ?>p</div></th>
  <th><div align="right"><? echo ($total_In - $total_Out); ?>p</div></th>
  <th><div align="right" style="color:#006666; "><? echo $total_collect; ?> p.</div></th>
</tr>
</tfoot>
</table>
<?php
...
function create_partner($userloginfirst,  $fio, $ph) {

 foreach ($_GET as $key => $value) {$$key = $value;}  foreach ($_POST as $key => $value) {	$$key = $value;}

	$result=sql_do("SELECT * FROM `users_ivr` ORDER BY `users_ivr`.`code`  DESC ");
				$row = mysql_fetch_array($result, MYSQL_BOTH);
				$code = $row['code'];
	...
	echo '<br>';
	$phh = md5($ph);
	$activationdate = date("20y-m-d");


			$res = sql_do("INSERT INTO `users_ivr` ( `user_id` , `userlogin` , `pass_hash` , `fio` , `mail` , `icq` , `tele_concact` , `additional` , `code` , `redir_num` , `vremya1` , `vremya2` , `partner_type`, `pwd`, `activationdate`, `startdate` ) VALUES ('', '$userlogin', '$phh', '$fio', '', '', '$tele_concact', '', '$code', '$redir_num', '0', '0', '$userloginfirst', '$ph' , '$activationdate', '$startdate' )");
...
/*                IVR  marazm           
if ($date >= '2008-04-01 00:00:00') {
	if ($type == 'Физическое лицо') return round(( $in * 0.9 * 0.86 ), 2);
...
/* 	if ($tarif_number == '9012029438') 	{
		if ($type == 'Юридическое лицо') $CostOut  =  ($region == 'cent') ? (19/60)*$duration  : (17/60)*$duration  ;
		else $CostOut  =  ($region == 'cent') ? (16.3/60)*$duration  : (14.3/60)*$duration  ;
	}
  	elseif ($tarif_number == '9012028013') 	{
		if ($type == 'Юридическое лицо') $CostOut  =  ($region == 'cent') ? (9.5/60)*$duration  : (8.5/60)*$duration  ;
...
<br>
Прибыль с <input name="profitfrom" type="text" size="6" value="<?php echo $profitfrom = isset($_POST['profitfrom'])?$_POST['profitfrom']:0; ?>"> по <input name="profitto" type="text" size="6" value="<?php echo $profitto = isset($_POST['profitto'])?$_POST['profitto']:100000; ?>"> рублей (может быть и отрицательной!)<br>

Минуты с <input name="durationfrom" type="text" size="6" value="<?php echo $durationfrom = isset($_POST['durationfrom'])?$_POST['durationfrom']:0; ?>"> по <input name="durationto" type="text" size="6" value="<?php echo $durationto = isset($_POST['durationto'])?$_POST['durationto']:1000000; ?>"><br>
...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>Спец.</th><th>Логин</th><th>Звонков</th><th>сек</th><th>c/60</th><th>мин</th><th>Доход</th><th>Расх.</th><th>Прибыль</th><th>Проц.</th><th>Ср.дл.</th></tr>
</thead>
<tbody>
<?
//exit;   
...
//echo "SELECT * FROM temp_user_data WHERE profit>'$profitfrom' AND profit<'$profitto' AND duration>'$durationfrom' AND duration<'$durationto' ORDER BY userlogin ";                 mins
...
$result= sql_do("SELECT * FROM temp_user_data WHERE profit>='$profitfrom' AND profit<='$profitto' AND mins>='$durationfrom' AND mins<='$durationto' ORDER BY userlogin ");
...
/*  if ($_POST['partnertype1'] &&  substr($row['userlogin'], 0, 1 ) == $_POST['partnertype1']  ) $letshow = true;
 ...
 if ( !$_POST['partnertype1'] && !$_POST['partnertype2'] && !$_POST['partnertype3'] && !$_POST['partnertype4']  ) */ $letshow = true;
     ...
     if ($letshow) {
			...
			$que = "SELECT * FROM `users_ivr` WHERE userlogin = '$userlogin'";
 			$resuser = sql_do($que);
			$rowuser = mysql_fetch_array($resuser, MYSQL_BOTH);
			$stoped = $rowuser['stoped'];
			$markstoped = ($stoped == 'Y') ? '<font color="#666666" size="2">' : '<font size=2>';
		...
		echo "<td><a href='index.php?mode=by_partner&userlogin=".$userlogin."' title='Посмотреть статистику партнера'>".$markstoped.$userlogin."</font></a></td>";
		echo "<td>".$calls."</td>";
		echo "<td>".$duration."</td>"."<td>".ceil($duration/60)."</td>"."<td>".$mins."</td>";      
		echo "<td>".$in."</td>";
		echo "<td>".$out."</td>";
		echo "<td>".$profit."</td>";
		echo "<td>".($in?(100 * round($profit/$in, 2)):0)." %</td>";
		echo "<td>".($calls ? round((ceil($duration/60)/$calls),1) : 0)."</td>";
		echo "</tr>";
 ...
 <tfoot>
<tr>
  <th colspan="5"><div align="right">Длительность: <? echo ceil($total_dur/60); ?> с/60</div></th>
  <th><div align="right"><? echo $total_mins; ?> m</div></th>
  <th><div align="right"><? echo $total_In; ?>p</div></th>
  <th><div align="right"><? echo $total_Out; ?>p</div></th>
  <th><div align="right"><? echo ($total_In - $total_Out); ?>p</div></th>
  <th>&nbsp;</th>
...
while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		...
		echo "<td>".$partner_num."</td>";
		echo "<td>".$row['abon_number']."</td>";
		echo "<td>".$duration."</td>"."<td>".($mins = ($duration < 11 || $row['abon_number'] == '9012029706') ? 0 : ceil($duration/60))."</td>";
		echo "<td>".$in."</td>";
...
function CrateTempUsersData ($datefrom,$dateto){
 ...
 sql_do($q);
 $q = "CREATE TABLE `temp_user_data` (  `id` bigint(20) NOT NULL auto_increment,  `origname` varchar(255) NOT NULL default '',  `userlogin` varchar(32) NOT NULL default '', `calls` bigint(20) NOT NULL default '0',  `duration` bigint(32) NOT NULL default '0',  `mins` bigint(32) NOT NULL default '0',  `in` float NOT NULL default '0',  `out` float NOT NULL default '0',  `profit` float NOT NULL default '0',   `outwithfrod` float NOT NULL default '0',   `result_out` float NOT NULL  default '0',    PRIMARY KEY  (`id`)) ENGINE=MyISAM DEFAULT CHARSET=cp1251 AUTO_INCREMENT=1 ";
 sql_do($q);
 ...
 while ($rowusers = mysql_fetch_array($resultusers, MYSQL_BOTH)){
	...
	if (strlen($userlogin)>4) continue;
...
//echo "SELECT * FROM totaldata WHERE  date>'$datefrom' AND date<'$dateto'  AND userlogin = '$userlogin'  ORDER BY date desc  = ".mysql_num_rows($result)."<br>";
	...
	while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		...
		$total_In 			= 	$total_In 			+ 	$in;
		$total_Out 			= 	$total_Out 			+ 	$out;
		$total_result_out 	= 	$total_result_out 	+ 	$result_out;
...
#		echo "INSERT INTO `temp_user_data` ( `id` , `origname` , `userlogin` , `calls` ,  `duration` , `in` , `out` , `profit` ) VALUES ('', '$origname', '$userlogin', '$i', '$duration', '$in', '$out', '$profit')<br>";
		...
		sql_do("INSERT INTO `temp_user_data` ( `id` , `origname` , `userlogin` , `calls` ,  `duration` ,  `mins` , `in` , `out` , `profit`, `outwithfrod`, `result_out` ) VALUES ('', '$origname', '$userlogin', '$i', '$total_dur', '$total_mins', '$total_In', '$total_Out', '$total_Profit', '$total_Outwithfrod', '$total_result_out')");
...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>ID</th><th>Подключение</th><th>Дата заполнения</th><th>ФИО</th><th>Статус</th><th>Специальность</th><th>Юр/Физ</th><th>Mail</th><th>WM</th><th>Т-ф</th><th>тел</th><th>С</th><th>По</th><th>Удаление</th></tr>
</thead>
<tbody>
<?
$and = " AND ";
...
while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		...
		echo "<td width='140px'>".($row['payed']?'ОПЛАЧЕНО':' - ')."</td>";
...
$origname = ( $_POST['change_radio'] == 'professionlist' )?  $_POST['origname_list']  : $_POST['origname_new']  ;
 ...
 if (!$condition && $_POST['activate']) echo $userlogin = $userloginfirst.$newcode;
	...
	echo '<br>';
 if ( ($newcode <= 0 || $newcode > 9999999999) && (!$condition && $_POST['activate']) ) {
	...
	if (mysql_num_rows($result) < 2) {
			...
			$res = 1; /* sql_do("INSERT INTO `users_ivr` ( `user_id` , `userlogin` , `pass_hash` , `fio` , `mail` , `icq` , `tele_concact` , `additional` , `code` , `redir_num` , `vremya1` , `vremya2` , `partner_type`, `type`, `wmpurse`, `activationdate`, `activationtime`  ) VALUES ('', '$userlogin', '$pass_hash_old', '', '', '', '', '', '$newcode', '$redir_num', '0', '0', '$userloginfirst', '$type', '$wmpurse', '$activationdate', '$activationtime' )"); */
			if(!$res) {
...
<b>Рады сообщить, что Ваш номер подключен! </b><br><br>

Для Вашей услуги был выделен  номер 8-809-505-'.$userloginfirst.'-'.$newcode.'. В рекламных материалах необходимо указывать:<br>


             - Услуга платная.  Точную стоимость уточняйте у Вашего оператора;<br><br>
...
Нашей службой протестирована работа Вашего номера, и теперь Вы можете приступить к работе. Настоятельно рекомендуем Вам также протестировать его самостоятельно во избежание накладок или недоразумений.<br>
Если в процессе тестирования работы своего номера Вы столкнулись с проблемами, просьба выслать на наш почтовый ящик подробную информацию: <br>
дата и время звонка, <br>
телефонный номер и название оператора связи, с которого был совершен тестовый вызов. <br>
Только при условии предоставления данной информации мы заводим техническую заявку и производим повторную проверку Вашего номера.<br>
Настоятельно рекомендуем не использовать переадресацию, автоответчик или голосовую почту для работы с нашим Сервисом. Если Вы не знаете подключены ли эти услуги, обратитесь в абонентскую службу своего оператора мобильной связи.
<br><br>

Советуем Вам проводить как можно более активную рекламную кампанию, иначе результативность Вашей работы будет низкой. Мы рекомендуем Вам использовать такие виды рекламы как: сайт-визитка, поисковая и тематическая реклама в системах Яндекс.Директ, Google Adwords и Бегун, реклама своих услуг на форумах и досках объявлений, реклама в местных газетах объявлений, а также любые другие доступные Вам формы рекламы.<br>
<br>
Так же предлагаю ссылки для бесплатных объявлений:<br>
<br>
Доски объявлений - все эти ресурсы имеют высокую посещаемость, сотрудничают <br>
с другими сайтами, т.е. есть вероятность, что объявления будут <br>
транслироваться и на других сайтах.<br>
<br>
<a href="http://www.slando.ru" target="_blank">http://www.slando.ru</a> - доска бесплатных объявлений без регистрации - легко <br>
выбрать раздел, тематику, указать способ связи.<br>
<br>
<a href="http://doska.us" target="_blank">http://doska.us</a> - доска бесплатных объявлений без регистрации - выбирается <br>
рубрика и потом текст объявления.<br>
<br>
<a href="http://www.vsedoski.ru" target="_blank">http://www.vsedoski.ru</a> - доска бесплатных объявлений без регистрации, <br>
добавляется текст объявления, по мере заполнения указывается тематика, срок <br>
размещения объявления.<br>
<br>
<a href="http://board.forenet.info" target="_blank">http://board.forenet.info</a> - тематический сайт форум регионов<br>
<br>
<a href="http://www.vtorgrad.ru" target="_blank">http://www.vtorgrad.ru</a> - можно разместить объявление<br>
<br>
Форум<br>
<br>
<a href="http://www.forum-opros.ru" target="_blank">http://www.forum-opros.ru</a> - регистрация, потом можно оставить информацию об <br>
услуге в близкой вам теме, можно добавить свою тему.<br>
<br>
...
<form action="../csvparse809.php" target="_blank">
...
<form action="../del_duplicates809.php" target="_blank" method="post">
...
<form action="../append_total_tab809.php" target="_blank">
	<input type=submit value="Дополнить Таблицу 2"> и дождаться полной загрузки
</form>
</fieldset>
<br>
<fieldset><legend>Альтернативный Шаг №3 (не применять!!!)</legend>
<form action="../total_tab809.php" target="_blank">
...
<form action="../total_balance809.php" target="_blank">
 ...
 */?><br>
Секунды с <input name="durationfrom" type="text" size="6" value="<?php echo $durationfrom = isset($_POST['durationfrom'])?$_POST['durationfrom']:0; ?>"> по <input name="durationto" type="text" size="6" value="<?php echo $durationto = isset($_POST['durationto'])?$_POST['durationto']:1000000; ?>"><br>
Деньги с <input name="profitfrom" type="text" size="6" value="<?php echo $profitfrom = isset($_POST['profitfrom'])?$_POST['profitfrom']:0; ?>"> по <input name="profitto" type="text" size="6" value="<?php echo $profitto = isset($_POST['profitto'])?$_POST['profitto']:100000; ?>"> руб.<br>
...
/* echo */ $q_223 = "SELECT * FROM `temp_user_data`   ORDER BY `result_out` DESC  ";
...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>Логин</th><th>ЛК</th><th>Текущий Баланс<?php /*  на <?=$dateto?> */?></th><th>сек</th><th>мин</th><th>Вознагр.</th><th>Вознагр. с фродом</th><th>Действия</th><th>ЮрСтатус</th></tr>
</thead>
<tbody>
<?
//exit;
while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
 ...
 if ( !$_POST['partnertype1'] && !$_POST['partnertype2'] && !$_POST['partnertype3'] ) $letshow = true; */
     ...
     if (/* $letshow && substr($row['userlogin'], 0, 1 ) != '4' */ 1 ) {  //  то есть это не  рекламная компания (4)
			$userlogin = $row['userlogin'];
...
//		echo "<td>".$row['origname']."</td>";
		...
		?><td><form action="../adm_cabinet809.php" method="POST" target="_blank"><input name="partnerlogin" type="hidden" value="<?=$row['userlogin']?>"><input type=submit value="ЛК <?=$row['userlogin']?>"></form></td><?
...
//		echo "<td>".$profit."</td>";
	  ...
	  echo '<td>';
		...
		elseif ( $started && $complete=='ZAK' && $type == 'Физическое лицо' ) echo '<font color="#00aa00">Заказано</font><br> <a href="index.php?mode=payitout&period='.$datefrom.'&userlogin='.$userlogin.'&out='.($row543['out'] /* < $outwithfrod ? $bal : $bal */).'">Оплачено</a> ('.round($row543['out'],2).'р., отправить на кошелек '.round($row543['out']*1.008,2).')';
		...
		elseif ( $started && $complete=='ZAK' && $type != 'Физическое лицо' && $type != 'ВебМани' ) echo '<font color="#00aa00">Заказано</font><br> <a href="index.php?mode=payitout&period='.$datefrom.'&userlogin='.$userlogin.'&out='.($row543['out'] /* < $outwithfrod ? $bal : $outwithfrod */).'">Оплачено</a> ('.round($row543['out'],2).'р.) '.$type;


		elseif ( $started && !$complete ) echo 'Начислено, не заказано,<br> ждем-с... <a href="index.php?mode=payitout&period='.$datefrom.'&userlogin='.$userlogin.'&out='.$bal/* ($bal < $outwithfrod ? $bal : $outwithfrod) */.'">или оплатить?</a>';
		else {
			echo '<a href="index.php?mode=checkfrod&userlogin='.$userlogin.'&datefrom='.$datefrom.'&dateto='.$dateto.'">Отметить фрод</a><br>';
			echo '<a href="index.php?mode=inprogress&period='.$datefrom.'&userlogin='.$userlogin.'&out='.$bal/* ($bal < $outwithfrod ? $bal : $outwithfrod) */.'">Начислить</a>';
		}
	  echo '</td>';
	echo "<td>".$type."</td>";
		echo "</tr>";
...
#----------------------------------------------------------------------------------------------------------------------------#
		...
		if ($plus || $minus || $payout || $result_out){
/* 			echo "<tr>";
			echo "<td>".$date."</td>";
			echo "<td>".$plus."</td>";
			echo "<td>".$result_out."</td>";
			echo "<td>".$minus."</td>";
			echo "<td>".$payout."</td>";
			echo "<td>".$comment."</td>";
			echo "</tr>";
...
$balance = $T_result_out - $T_minus - $T_payout;
return $balance;
...
$qq = "INSERT INTO `payouts` ( `id` , `user_id` , `userlogin` , `year` , `month` , `out` , `payoutdate` , `period`, `complete` ) VALUES ( '', '', '$userlogin', '".$date_array[0]."', '".$date_array[1]."', '".$out."', '20".date("y-m-d")."', '$period', '' )";
sql_do($qq);
...
<?
		...
		echo '<br><a href="'.$back.'">Вернуться назад</a>';
