...
<?
	while ($ro3 = mysql_fetch_array($result3, MYSQL_BOTH)){
		...
		if (1) 
		{
//			Lg_Button(4, $napominanie, $skript."?mode=edit_lgotygiltsa", "post", "edit_lgotygiltsa", substr($nazvanie, 0, 20)  , $id, $idlevel[1], $idlevel[2], $idlevel[3]);
			Uni_Button(4, $napominanie, $skript."?mode=edit_lgotygiltsa", "post", "edit_lgotygiltsa", substr($nazvanie, 0, 20)  , $id, $idlevel);
			if( ($ii%$columns) ) {?></td><td><? }
			if( !($ii%$columns) ) {  
				?></td></tr><tr><td><? 
			}
...
<?
//	Lg_Button(4, "", $skript."?mode=create_lgotygiltsa", "post", "create_lgotygiltsa","Создать льготу", 0, $idlevel[1], $idlevel[2], $idlevel[3]);
	Uni_Button(4, "", $skript."?mode=create_lgotygiltsa", "post", "create_lgotygiltsa","Создать льготу", 0, $idlevel);
?>  
</fieldset>
<? 
}
