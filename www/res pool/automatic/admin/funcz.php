...
else {$skript = 'cabinet.php'; $arr2 = $partnerlogin = $_POST["partnerlogin"] = $_POST["login"];}
...
//		var query = '' + document.getElementById('query').value;
    ...
    var q_oe = oe;
		...
		req.open('POST', 'admin/ajax_delvidimost.php?test=abc', true);
...
<div id="debug" style="display:none; border:2px solid red; margin:2px; padding:5px;	 width: 850px; left: 164px; background-color:#000099; color:#FFFF00;  z-index:10; "></div>
<div id="result"></div>
<?
...
echo'<table  class="printview sortable"><thead>
  <tr>
    <th scope="col">Видимость в таблицах</th>
...
';
	...
	$qooo = "SELECT * FROM `".$_POST["partnerlogin"]."__ooo`  ORDER BY `vidimost_yn` ASC , `naimenovanie` ASC ";
	$resooo = mysql_query($qooo)   or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<br>qooo:<BR>".$qooo."<BR>") ;
	while ($rowooo = mysql_fetch_object($resooo)){
	    $id 	= $rowooo->id;
	    $naimenovanie 	= $rowooo->naimenovanie;
	    $vidimost_yn 	= $rowooo->vidimost_yn;
		
	$button = /* $idlevel_1.' ----  '.$idlevel_2. */
	'<form  id="site'.$id.'"  action="?mode=edit_ooo" method="post" enctype="application/x-www-form-urlencoded"   target="_blank">
  <input name="dopolnitelno" value="'.$idlevel_2.'" type="hidden">
    <input name="idlevel_1" value="'.$idlevel_1.'" type="hidden">
	
	<input name="tablelevel" value="2" type="hidden">  
  <!--button class="mybut2" title="">изменить</button-->
  <div class="mybut2" title=""  onClick="check99(\'site'.$id.'\');">добавить</div>
  </form>';
...
$arr = $arr0."$$$".$arr1."$$$".$arr2."$$$";
	...
	echo'  
   		  <tr>
		    <td title="сделать невидимым (нажать на все ненужные, значёк изменится после обновления странички)">';

	echo "<div 
				id=\"id_".$idlevel_2."\" 
				onclick=\"doLoad(true, 'result_".$id."', 'debug', '".$idlevel_2."', '".$name."', '".$arr."', 'id_".$idlevel_2."')\" 
				style=\" border:0px solid red; cursor:pointer;  font-weight:bold; font-size:12px; \"
		>выключить</div>";
	}
	else {
	$name = 'on';
	echo'  
   		  <tr>
		    <td title="сделать видимым (нажать на все нужные, значёк изменится после обновления странички)">';

	echo "<div 
				id=\"id_".$idlevel_2."\" 
				onclick=\"doLoad(true, 'result_".$id."', 'debug', '".$idlevel_2."', '".$name."', '".$arr."', 'id_".$idlevel_2."')\" 
				style=\" border:2px solid red; cursor:pointer;  font-weight:bold; font-size:12px; \"
		>включить</div>";
	} 


			echo '</td>
		    <td title="'.$naimenovanie.'">
				<div align="left">'.$naimenovanie.'</div></td>
		    
		    <td title="check">
				<div  id=result_'.$id.'   align="left">???</div></td>
		    </tr>
	';
...
//		var query = '' + document.getElementById('query').value;
    ...
    var q_oe = oe;
		...
		req.caching = false /* true */;
		// Подготваливаем объект.
		req.open('POST', 'admin/ajax_delsvodnaya.php?test=abc', true);
		// Посылаем данные запроса (задаются в виде хэша).
		req.send({ q: query, rr: query1, b: q_bn, i: q_io, o: q_oe, test:303 });
	}
	// Поддержка загрузки данных по тайм-ауту (1 секунда после
...
$qqq1 = "SELECT * FROM `".$_POST["partnerlogin"]."__juradres`  ORDER BY `idlevel_2` ASC , `id` DESC ";
...
echo'<table  class="printview sortable"><thead>
  <tr>
    ...
    <th scope="col">Адрес ООО</th>
    <th scope="col">Аренда</th>
    <th scope="col">ПСО</th>
    <th scope="col">Писем</th>
  </tr></thead><tbody>
';
...
while ($row1 = mysql_fetch_object($Or1)){
	...
	if ($idlevel_2 == $row1->idlevel_2) continue; #break;
	...
	$qooo = "SELECT * FROM `".$_POST["partnerlogin"]."__ooo`  WHERE idlevel_1 ='$idlevel_1' AND  id ='$idlevel_2'";
	$resooo = mysql_query($qooo)   or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<br>qooo:<BR>".$qooo."<BR>") ;
	while ($rowooo = mysql_fetch_object($resooo)){
	    $telefon 	= $rowooo->telefon;
	    $pochtindex 	= $rowooo->pochtindex;
    	$subyektrf 	= $rowooo->subyektrf;
	    $subject 	= $rowooo->subject;
	    $rajon 	= $rowooo->rajon;
	    $gorod 	= $rowooo->gorod;
	    $naspunkt 	= $rowooo->naspunkt;
	    $ulitca 	= $rowooo->ulitca;
	    $dom 	= $rowooo->dom;
	    $kvartira 	= $rowooo->kvartira;
	    $email 	= $rowooo->email;
	    $svodnaya_yn 	= $rowooo->svodnaya_yn;
	    $vidimost_yn 	= $rowooo->vidimost_yn;
	    $inn 	= $rowooo->inn;
		
//		$oooaddr = $pochtindex .", ". /* $subyektrf .", ". */ $subject .", ". $rajon .", ". ($gorod=="Москва"?"":$gorod .", "). $naspunkt .", ". $ulitca .", ". $dom .", ". $kvartira ;
...
#    $given_yn 	= ($row1->given_yn=='нет' || $row1->given_yn=='0')?'<font color="#FF0000">нет</font>':$row1->given_yn;
  ...
  <div class="mybut2" title=""  onClick="check99(\'site'.$id.'\');">добавить</div>
	...
	$Or1qqopja = mysql_query($qqopja)   or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<br>qqopja:<BR>".$qqopja."<BR>") ;
...
########################################################## Обработка даты продления #########################################################
	...
	if ($enddate <= date("20y-m-d")) 
		$enddate1 = '<font color="#FF0000" style="text-decoration: underline;">'.$enddate.'</font>';
	else 
		$enddate1 = '<font color="#0000ff" style="text-decoration: underline;">'.$enddate.'</font>';
###############################################################################################################################################


$qcorr = "SELECT * FROM `".$_POST["partnerlogin"]."__correspondence`  WHERE idlevel_1 ='$idlevel_1' AND  idlevel_2 = '$idlevel_2' AND `given_yn` != 'да'  ORDER BY `receive_d` DESC ";
$rqcorr = mysql_query($qcorr)   or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<br>qcorr:<BR>".$qcorr."<BR>") ;
$numcorr=0;
	while ($rowcorr = mysql_fetch_object($rqcorr)){
	    $numcorr++;
	}
...
$imya = '';
$otchestvo = '';
$qpers = "SELECT * FROM `".$_POST["partnerlogin"]."__ooopersons`  WHERE idlevel_1 ='$idlevel_1' AND  idlevel_2 = '$idlevel_2' AND `gendir_yn` = 'да'";
$rpers = mysql_query($qpers)   or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<br>qpers:<BR>".$qpers."<BR>") ;
$num_rows = mysql_num_rows($rpers);

if($num_rows)
  {
	while ($rowrpers = mysql_fetch_object($rpers)){
	     $familya 	= $rowrpers->familya;
	     $imya 	= $rowrpers->imya;
	     $otchestvo 	= $rowrpers->otchestvo;
...
$arr = $arr0."$$$".$arr1."$$$".$arr2."$$$";
	...
	if($adress != $user && $svodnaya_yn == 'да' && $vidimost_yn == 'да'){
	echo'  
   		  <tr>
		    <td title="удаление (нажать на все ненужные, пропадут после обновления странички)">';
//			echo '<div align="left">х</div>';

	echo "<div 
				id=\"id_".$idlevel_2."\" 
				onclick=\"doLoad(true, 'result_".$idlevel_2."', 'debug', '".$idlevel_2."', '".$name."', '".$arr."', 'id_".$idlevel_2."')\" 
				style=\" border:0px solid red; cursor:pointer;  font-weight:bold; font-size:12px; \"
		>х</div>";
		    ...
		    <td><div align="center">'.($inn?$inn:$button).'</div></td>
		    <td><div align="center">'.$fio.'</div></td>
		    <td><div align="center">'.$telefon.'</div></td>
		    <td><div align="center">'.$oooaddr.'</div></td>
		    <td><div align="center">'.$enddate1.'</div></td>
		    <td><div align="center">'.$pso_yn.'</div></td>
		    <td><div align="center">'.($numcorr).'</div></td>
		  </tr>
	';
	}
}
...
if ($allletters == 'all') $qqq1 = "SELECT * FROM `".$_POST["partnerlogin"]."__correspondence`  ORDER BY `receive_d` DESC";

elseif ($allletters == 500) $qqq1 = "SELECT * FROM `".$_POST["partnerlogin"]."__correspondence`  WHERE `receive_d`
BETWEEN '2013-01-01' AND '2020-12-31' ORDER BY `receive_d` DESC LIMIT 0 , 500";

else $qqq1 = "SELECT * FROM `".$_POST["partnerlogin"]."__correspondence`  WHERE `receive_d`
BETWEEN '2013-01-01' AND '2020-12-31' ORDER BY `receive_d` DESC LIMIT 0 , 100";
...
<table width="200" border="0">
  <tr>
    <td><form action="adm_cabinet.php?mode=pso" method="post" name="skolko" target="_blank">
<input name="allletters" type="hidden" value="all">
<input name="all" type="submit" value="все письма">
</form>
</td>
    <td><form action="adm_cabinet.php?mode=pso" method="post" name="skolko" target="_blank">
<input name="allletters" type="hidden" value="500">
<input name="all" type="submit" value="500 писем">
</form></td>
  </tr>
</table>
...
$text = ' Уведомляем, что для Вашей компании  "'.$ooo_name.'"  поступила корреспонденция. <br>Отправитель: '.$fromp.'. 
<br><br>
Если у Вашей компании нет действующего Договора на почтово-секретарское обслуживание, забрать письмо возможно оплатив разовую услугу почтово-секретарского обслуживания (получение, хранение и выдача почтовой корреспонденции) в размере 500 руб.
Если у Вашей компании нет действующего Договора аренды, то прежде всего его необходимо будет пролонгировать на новый срок. 
<br><br>
Получить корреспонденцию можно в нашем офисе в любой будний день (с 9 до 17 в пн-чт, с 9 до 16 в пт) по адресу: г. Москва, п. Московский, 22 км Киевского ш., домовл. 4 стр. 3 корп. Д оф. 610Д 
Просим предварительно звонить и заказывать пропуска по телефону (действует пропускной режим, просьба иметь при себе документ, удостоверяющий личность).
<br><br>
По всем вопросам, связанным с получением почтовой корреспонденции, пишите нам на inteh.ooo@mail.ru
<br><br>
ООО Интех
+7(495) 96-96-456
';
...
while ($row1 = mysql_fetch_object($Or1)){
    ...
    $payed 			= $row1->payed;
					...
					//   проверки на видимость, если невидимо - проскакиваем цикл   //
	$qooo = "SELECT * FROM `".$_POST["partnerlogin"]."__ooo`  WHERE idlevel_1 ='$idlevel_1' AND id = '$idlevel_2' AND vidimost_yn = 'да'   ";
	$resooo = mysql_query($qooo)   or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<br>qooo:<BR>".$qooo."<BR>") ;
	if(!mysql_num_rows($resooo)) continue;
#--------------------------------------------------------------------------------------------------------------
...
echo'<table width="90%"  class="printview">
  <tr>
    ...
    <th scope="col"><div align="center">Дробь</div></th>
  </tr>
';
...
if ($dolyaprocent && (!$drob1 && !$drob2)) 	$dolyanominal = $kapital*$dolyaprocent/100;
elseif ($drob1 && $drob2 && !$dolyaprocent) 	$dolyanominal = $kapital*$drob1/$drob2;
else $dolyanominal = "ошибка!!!!!!!";
	...
	echo'  
		    ...
		    <td><div align="center">'.$drob1.'/'.$drob2.'</div></td>
		  </tr>
	';
...
<input name="drob1" 		type="hidden" value="'.$drob1.'">
<input name="drob2" 		type="hidden" value="'.$drob2.'">
...
if ($num_rows>1) $co=ceil(  ($num_rows-1)/56    );
elseif ($num_rows==1) $co=1;
else echo "ОКВЭД отсутствует!!!";


$page = $page*2 + $co;

#echo "okv 2 page = ".$page." , num_rows = ".$num_rows." , co = ".$co;
...
'.SubmitButton('Доверенность '.$familya.' '.$imya.' '.$otchestvo.'', '', '').'  
...
if ($adrold) echo '<form action="print/oooprotokol-adrold.php" method="post" target="_blank">';

else echo '<form action="print/oooprotokol.php" method="post" target="_blank">';
...
<!--button  class="small magenta awesome" title="Протокол">Протокол адрес по старому</button-->  
'.SubmitButton('Протокол адр.стар.', '', '').'  
...
<form action="../automatic_utf8/tcpdf/examples/oookvitanciya.php" method="post" target="_blank">
...
<!--button  class="small blue awesome" title="Квитанции">Квитанции штрих-код</button-->  
'.SubmitButton('Квитанции штрих-код', '', '').'  
...
#####################   Устав          #####################
############################################################
if ($adrold) echo '<form action="print/oooustav-adrold.php" method="post" target="_blank">';

else echo '<form action="print/oooustav.php" method="post" target="_blank">';
...
<!--button  class="small red awesome" title="Устав">Устав адр.стар.</button-->  
'.SubmitButton('Устав адр.стар.', '', '').'  
...
<form action="../automatic_utf8/tcpdf/examples/ipkvitanciya.php" method="post" target="_blank">
...
<!--input name="zgo" type="submit" value="Распечатать Квитанцию"-->
'.SubmitButton('Квитанция штрих-код', '', '').'  
...
#-------------------------------------------------------------------------------------------------------------------------------------
#--------------------------------------------    function massedit2014_okved($user_id)     ---------------------------------------------
#-------------------------------------------------------------------------------------------------------------------------------------
...
$query31="SELECT * FROM `okved2014full` ORDER BY na_ekrane ASC, id ASC";
$result = mysql_query ($query31) or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<BR>".$query31."<BR>");
$totalRows_result = mysql_num_rows($result); 
...
elseif ($get_mode[0] == "delete" || $get_mode[0] == "confirmdelete" || $get_mode[0] == "massedit" || $get_mode[0] == "massedit2014" || $get_mode[0] == "print" || $get_mode[0] == "upload" || $get_mode[0] == "emailooo")   	
	{
		$id = $_POST['dopolnitelno']; 
	}
#----------------------------------------------------------------------------------------------------------------
