...
<?
$que = "SELECT * FROM `users_jkh` WHERE partner_type != '4' && mail != '' && fio != ''";
 	$result = sql_do($que);
...
#-------------------------
  ...
  if ($_POST["userlogin"] && $_POST['b'.$_POST["userlogin"]] == 'Go!') {
		$qUserUpd = "UPDATE `users_jkh` SET `document` = '$document', `agreement` = '$agreement', `requisition` = '$requisition', `inn` = '$inn' WHERE `userlogin` = '".$_POST["userlogin"]."'";
		sql_do($qUserUpd);
  }
...
$que = "SELECT * FROM `users_jkh` WHERE `redir_num` = '9012029438' ORDER BY `type` ASC , `userlogin` ASC ";
$result = sql_do($que);
