...
<?
	while ($ro3 = mysql_fetch_array($result3, MYSQL_BOTH)){
		...
		if (1) 
		{
//			Gl_Button(3, $napominanie, $skript."?mode=edit_giltsy", "post", "edit_giltsy", $familiya." ".$imya." ".$otchestvo, $id, $idlevel[1], $idlevel[2], $idlevel[3]);
			Uni_Button(3, $napominanie, $skript."?mode=edit_giltsy", "post", "edit_giltsy", $familiya." ".$imya." ".$otchestvo, $id, $idlevel);
			if( ($ii%$columns) ) {?></td><td><? }
			if( !($ii%$columns) ) {  
				?></td></tr><tr><td><? 
			}
...
<?
//	Gl_Button(3, "", $skript."?mode=create_giltsy", "post", "create_giltsy", "Создать жильца", 0, $idlevel[1], $idlevel[2], $idlevel[3]);
	Uni_Button(3, "", $skript."?mode=create_giltsy", "post", "create_giltsy", "Создать жильца", 0, $idlevel);
?>  
</fieldset>
<? 
}
