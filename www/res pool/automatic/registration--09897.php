...
//	echo "<font color='#ffffff' size='2' face='Arial'>Отправка регистрационных данных...<br><br>";      details
		...
		if ( isset($_POST['mail']) &&!empty($_POST['mail'])  && preg_match("/^(?:[a-z0-9]+(?:[-_]?[a-z0-9]+)?@[a-z0-9]+(?:\.?[a-z0-9]+)?\.[a-z]{2,5})$/i", trim($_POST['mail'])  )    ) { ; } 
...
$qins2 = "CREATE TABLE IF NOT EXISTS `".$_POST['id']."__ip` (
  ...
  `level` varchar(255) NOT NULL COMMENT 'invisible 2 физлицо',
...
$qins3 = "CREATE TABLE IF NOT EXISTS `".$_POST['id']."__ooopersons` (
  ...
  `objrusname` varchar(20) NOT NULL COMMENT 'invisible участниками_ООО',
  KEY `id` (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=cp1251 AUTO_INCREMENT=1 ";
			sql_do($qins3);
