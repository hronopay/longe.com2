...
<h3>Админпанель <font color="#FF0000" style="text-decoration:blink; ">короткие</font></h3>
<?
...
switch ($mode)
	...
	case "prefix": echo "Управление префиксами:"; stats_Prefix($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;

	case "prefix_old": echo "Сводная по звонкам:"; stats_PrefixOLD($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;
	...
	case "files": echo "Файлы"; files (); break;
	...
	case "reg_stat": echo "СТАТИСТИКА РЕГИСТРАЦИИ :"; RegStat ($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;
...
#	case "payouts": echo "Выплаты :"; PayOuts ($datefrom = make_date_c('from'), $dateto = MakeDateForNextMonth()); break;
	...
	case "checkfrod": echo "Отметить фрод НА МТС :"; CheckFrod ($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;

	case "checkBEEfrod": echo "Отметить фрод НА БИЛАЙН:"; checkBEEfrod ($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;
	...
	case "all_partners_last": echo "Быстрое упрвление партнёрами: Статус документов свежие"; all_partners_last (); break;
	...
	case "nopayed_partners": echo "Переписка"; nopayed_partners (); break;

	case "oplachenopodkl": echo "Пометили как оплаченного"; oplachenopodkl (); break;

	case "oplachenopodkl_2": echo "Пометили как оплаченного"; oplachenopodkl_2 (); break;

	case "mtsforfrod": echo "МТС для фрода:"; mtsforfrod($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;

	case "beeforfrod": echo "Билайн для фрода:"; beeforfrod($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;

	case "megforfrod": echo "Мегафон для фрода:"; megforfrod($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;

	case "docscheck": echo "Проверка документов от партнеров:"; docscheck(); break;

	case "nlcalculator": echo "Сводная по звонкам:"; nlcalculator($datefrom = make_date_c('from'), $dateto = make_date_c('to')); break;

	case "operators": echo "Сводная по звонкам:"; operators(); break;
