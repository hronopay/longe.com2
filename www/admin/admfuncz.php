...
?>ИЛИ<br><br><form method=post action="index.php?mode=show_partner">
Выберите логин Неподключенного:
	<select name="userlogin"><?
 	while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		?><option value="<?=$row['userlogin'];?>"><?=$row['userlogin'];?></option><?
	}
...
function create_partner($userloginfirst, $code) {
	...
	echo '<br>';
  if ($code <= 1000 || $code > 5000) echo "<br><br>Не удалось создать логин $userlogin, так как выбран неверный формат префикса!!!";
  else {
	;
...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>N</th><th>Дата</th><th>Спец.</th><th>Логин</th><th>Префикс</th><th>переадр.</th><th>абонент.</th><th>сек</th><th>мин</th><th>Доход</th><th>Расх.</th><th>Прибыль</th><th>acn</th><th>Fr</th></tr>
</thead>
<tbody>
<?
//exit;
...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>ID</th><th>Дата заполнения</th><th>ФИО</th><th>Оплата WM</th><th>Статус</th><th>Подробности</th><th>Опл. пометить</th><th>Специальность</th><th>Юр/Физ</th><th>Mail</th><th>WM</th><th>тел</th><th>С</th><th>По</th><th>Удаление</th></tr>
</thead>
<tbody>
<?
$and = " AND ";
...
while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		...
		$status = $row['condition'] ? 'работает' : ($row['payed'] ?  '<font color="#ee0000">ожидание активации</font>' :'ожидание оплаты') ;
		$additional = ($row['additional'] == '') ? "&nbsp;" : $row['additional'] ;
		$additional = strstr($additional, 'МобилМани') ? '<font color="#ee0000">'.$additional."</font>" : $additional ; 
		...
		echo $row['payed'] ? ( $status == 'работает' ?  
		"<td>".'&nbsp;'."</td>"
		:
		"<td>".'<form action="'.$_SERVER['SCRIPT_NAME'].'?mode=oplachenopodkl" method="post" name="opl"><input name="id" type="hidden" value="'.$row['id'].'"><input name="opl_'.$row['id'].'" type="submit" value="Отм. опл." style="background-color:#FFCC00; font-size:10px;"><input name="do" type="hidden" value="0"></form>'."</td>" )
		: 
		"<td>".'<form action="'.$_SERVER['SCRIPT_NAME'].'?mode=oplachenopodkl" method="post" name="opl"><input name="id" type="hidden" value="'.$row['id'].'"><input name="opl_'.$row['id'].'" type="submit" value="Опл." style="background-color:#00CCFF; font-size:10px;"><input name="do" type="hidden" value="1"></form>'."</td>";
...
<br>
             ...
             - Длительность вызова округляется поминутно в большую сторону (для трафика, превышающего 100 минут в месяц);<br><br>
...
Нашей службой протестирована работа Вашего префикса, и теперь Вы можете приступить к работе. Настоятельно рекомендуем Вам также протестировать его самостоятельно во избежание накладок или недоразумений.<br><br>


Внимание: набор префикса должен производиться после окончания голосового файла!<br><br>

Если в процессе тестирования работы своего префикса Вы столкнулись с проблемами, просьба выслать через форму обратной связи в личном кабинете подробную информацию: 
<ul>
  <li>короткий номер и префикс которые набирали</li>
  <li>    дата и время звонка</li>
  <li>    сообщение системы после ввода префикса</li>
  <li>    телефонный номер и название оператора связи, с которого был совершен тестовый вызов</li>
</ul>
...
#                        ------------      Подключение номера (тестовый режим)       ------------
	...
	if( $row990['payed'] != 'YES'){
		sql_do("INSERT INTO `payins` ( `id` ,  `userlogin` , `in` , `payindate`, `comment`  ) VALUES ( '',  '$userlogin', '-990', '20".date("y-m-d")."', 'Подключение номера (тестовый режим)' )");
		sql_do("INSERT INTO `user_balans` VALUES ('', '$userlogin', '-990', 0, 0, 0, '20".date("y-m-d")."', 'Подключение номера (тестовый режим)')");
...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>Логин</th><th>ЛК</th><th>Баланс на <?=$dateto?></th><th>сек</th><th>мин</th><th>Вознагр.</th><th>Вознагр. с фродом</th><th>Действия</th><th>ЮрСт</th><th>fio</th></tr>
</thead>
<tbody>
<?
//exit;
...
//		echo "<td>".$row['origname']."</td>";
		...
		echo "<td><font size=2><a href='index.php?mode=by_partner&userlogin=".$userlogin."&datefrom=".$datefrom."&dateto=".$dateto."' title='Посмотреть статистику партнера' target='_blank'>".$userlogin."</a></td>";
...
$j = 0;
 while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
	if(aprox($row['duration'])>0){
		...
		echo "<td>".$duration."</td>"."<td>".ceil($duration/60)."</td>";
		echo "<td>".$out."</td>";
		echo '<td><input name="call_'.$session_id.'" type="checkbox" value="'.$session_id.'" '.$chdis.'>'.($chdis ? '<a href="index.php?mode=cancel_bee_frod&session_id='.$session_id.' ">снять отметку</a>' : '').'</td>';
		echo "</tr>";
		$total_dur=$total_dur+aprox($row['duration']);
		$total_In = $total_In + $in;
		$total_Out = $total_Out + $out;
...
<input type="hidden" name="datefrom" value="<?php echo $_POST['datefrom']; ?>">
<input type="hidden" name="dateto" value="<?php echo $_POST['dateto']; ?>">
	<input type=submit value="На БиЛайн-Фрод"> 
</form>
<? }
}
