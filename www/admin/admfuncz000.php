...
//$result = sql_do("SELECT * FROM incoming WHERE called_number='$redir_num'  AND (user_code='$code' OR user_code='' ) AND date>='$datefrom' AND date<='$dateto' ORDER BY date DESC");
$ququ = "SELECT * FROM incoming WHERE called_number='$redir_num'  AND (user_code='$code' OR user_code='' ) AND date>='$datefrom' AND date<='$dateto' ORDER BY date DESC";
...
function create_partner($userloginfirst, $code) {
	...
	echo '<br>';
  if ($code < 3100 || $code > 3199) echo "<br><br>Не удалось создать логин $userlogin, так как выбран неверный формат префикса!!!";
  else {
	open_connection();
...
if ($date >= '2008-07-01 00:00:00') {
	if ($type == 'Физическое лицо') return round(( $in * 0.9 * 0.90 ), 2);
	elseif ($type == 'ВебМани') return round(( $in * 0.9 * 0.93 ), 2);
	else  return round(($in*0.9), 2);
}
#---------------
...
#---------------------------Делаем 47% Уралвестком------------------------------------------------------
	
	if ($work == 14) $tarif_1 = 42;
...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>Спец.</th><th>Логин</th><th>Звонков</th><th>сек</th><th>мин</th><th>Доход</th><th>Расх.</th><th>Прибыль</th></tr>
</thead>
<tbody>
<?
//exit;
...
<?php
		if ($_GET['action'] == 'delete')  {
			...
			echo "<font color=#ff0000>Клиент успешно удален!</font><br>";
		}
?>
</fieldset>
<script src="../js/sorttable.js"></script>
...
while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		...
		echo "<td><a href='index.php?mode=regs&action=delete&user_id=".$row['user_id']."' title='Удалить регистрацию клиента'>Удалить</a></td>";
		echo "</tr>";
...
Нашей службой протестирована работа Вашего префикса, и теперь Вы можете приступить к работе. Настоятельно рекомендуем Вам также протестировать его самостоятельно во избежание накладок или недоразумений.<br>
Внимание: набор префикса должен производиться после окончания голосового файла! Если в процессе тестирования работы своего префикса Вы столкнулись с проблемами, просьба выслать на наш почтовый ящик подробную информацию: <br>
короткий номер и префикс которые набирали, <br>
дата и время звонка, <br>
сообщение системы после ввода префикса, <br>
телефонный номер и название оператора связи, с которого был совершен тестовый вызов. <br>
Только при условии предоставления данной информации мы заводим техническую заявку и производим повторную проверку Вашего префикса.<br>
...
<br>
Если Вы ещё не отправили необходимые сведения, то советуем Вам сделать это в ближайшее время. Подробности на странице "Инструкции" нашего сайта: <a href="http://mcall.ru/instr.php" target="_blank">http://mcall.ru/instr.php</a><br>
<br>
...
Тел/факс.:+7(499)503-10-18 <br>
117513, Москва, Ленинский пр-т, дом 123 корп.3<br>
...
/* echo */ $q_223 = "SELECT * FROM `temp_user_data` WHERE `out` > '$profitfrom' AND `out` < '$profitto' AND `duration` >'$durationfrom' AND `duration` <'$durationto' ORDER BY userlogin ";
...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>Логин</th><th>Вознагр.</th><th>Вознагр. с фродом</th><th>Действия</th></tr>
</thead>
<tbody>
<?
//exit;
while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
 ...
 if ( !$_POST['partnertype1'] && !$_POST['partnertype2'] && !$_POST['partnertype3'] ) $letshow = true;
     ...
     if ($letshow && substr($row['userlogin'], 0, 1 ) != '4') {
			$userlogin = $row['userlogin'];
...
//		echo "<td>".$profit."</td>";
		...
		echo '<td><a href="index.php?mode=checkfrod&userlogin='.$userlogin.'&datefrom='.$datefrom.'&dateto='.$dateto.'">Отметить фрод</a><br>';
		if ($payed) echo 'Оплачено';
		else echo '<a href="index.php?mode=payitout&period='.$datefrom.'&userlogin='.$userlogin.'&out='.$outwithfrod.'">Оплатить</a>';
		echo '<br><a href="index.php?mode=inprogress">Расчитывается</a></td>';
		echo "</tr>";
//		$total_dur=$total_dur + $duration;
...
sql_do("INSERT INTO `payouts` ( `id` , `user_id` , `userlogin` , `year` , `month` , `out` , `payoutdate` , `period` )
...
while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		...
		echo "<td>".$out."</td>";
		echo '<td>';
		echo '<a href="index.php?mode=cancelpayout&period='.$datefrom.'&userlogin='.$userlogin.'">Отменить оплату</a>';
		echo "</tr>";
		$total_Outwithfrod = $total_Outwithfrod + $outwithfrod;
		$total_Out = $total_Out + $out;
