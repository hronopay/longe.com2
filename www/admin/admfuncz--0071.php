...
<table cellspacing=0 border=1  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;  " class="sortable">
<thead>
	<tr><th>Логин</th><th>сек</th><th>мин</th><th>Вознагр.</th><th>Вознагр. с фродом</th><th>Действия</th></tr>
</thead>
<tbody>
<?
//exit;
...
//		echo "<td>".$profit."</td>";
	  ...
	  echo '<td>';
		if ( $started && $complete ) echo '<font color="#aa0000">Оплачено</font>';
		elseif ( $started && !$complete ) echo '<a href="index.php?mode=payitout&period='.$datefrom.'&userlogin='.$userlogin.'&out='.$outwithfrod.'">Пометить как "Оплачено"</a>';
		else {
			echo '<a href="index.php?mode=checkfrod&userlogin='.$userlogin.'&datefrom='.$datefrom.'&dateto='.$dateto.'">Отметить фрод</a><br>';
			echo '<a href="index.php?mode=inprogress&period='.$datefrom.'&userlogin='.$userlogin.'&out='.$outwithfrod.'">Начать оплату</a>';
		}
	  echo '</td>';
...
while ($row = mysql_fetch_array($result, MYSQL_BOTH)){
		...
		echo "<td>".$out."</td>";
		echo '<td>';
		echo ( ($complete == 'YES') ? 'Деньги отправлены (<a href="index.php?mode=cancelpayout&period='.$datefrom.'&userlogin='.$userlogin.'">все равно отменить</a>)' : '<a href="index.php?mode=cancelpayout&period='.$datefrom.'&userlogin='.$userlogin.'">Отменить начатую оплату</a>');
		echo '</td>';
		echo "</tr>";
		$total_Outwithfrod = $total_Outwithfrod + $outwithfrod;
		$total_Out = $total_Out + $out;
...
VALUES ( '', '', '$userlogin', '".$date_array[0]."', '".$date_array[1]."', '$out', '20".date("y-m-d")."', '$period', '' )");
 ...
 echo "<br><br>Партнеру с логином $userlogin <br>зафиксировано начало процесса выплаты в сумме $out рублей <br>за ".$date_array[1]." месяц  ".$date_array[0]." года.";
?>
<br><br>
Выплата производится на следующие реквизиты:<br><br>
...
?>
<br><br>
Можно  отметить что <strong>оплата завершена</strong> позже, а можно прямо отсюда - для этого <strong>после отправки денег</strong> нажать на  ссылку<br>
<br>
<?
