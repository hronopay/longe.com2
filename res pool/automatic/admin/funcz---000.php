...
$qqq1 = "SELECT * FROM `".$_POST["partnerlogin"]."__juradres`  ORDER BY `from_date` ASC ";
$Or1 = mysql_query($qqq1)   or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<br>qqq1:<BR>".$qqq1."<BR>") ;
...
########################################################## Обработка даты продления #########################################################
	$enddate = $from_date ? CalcDateForvard($from_date, $juradr_srok) :'&nbsp;';
	if ($enddate <= date("20y-m-d")) {
		$qend = "SELECT * FROM `".$_POST["partnerlogin"]."__juradres`WHERE  idlevel_2 = '$idlevel_2'   ORDER BY `from_date` ASC ";
		...
		while ($row3e = mysql_fetch_object($Oqend)){
		    if($id!=$row3e->id && $from_date<$row3e->from_date) {
				$enddate1 = 'продлён';//'<font color="#0000FF">'.$enddate.'</font>';
#				echo $id ." --$$$-- ". $from_date."<br>";
#				echo $row3e->id ." -$$- ". $row3e->from_date."<br>";
			}
			else {
...
########################################################## Обработка даты продления #########################################################
	...
	echo'  
   		  <tr>
		    <td title="'.$adress.'"><div align="left">'.$adress.'</div></td>
		    <td><div align="center">'.$from_date.'</div></td>
...
'.SubmitButton("Распечатать Заявление Начало", '', '').'  
</form>
';
...
'.SubmitButton('Лист Б (Учредитель) '.$familya.' '.$imya.' '.$otchestvo.'', '', '').'  
</form>
';
}
...
'.SubmitButton('Лист Е (Гендир)  '.$familya.' '.$imya.' '.$otchestvo.'', '', '').'  
</form>
';
}
...
'.SubmitButton('Лист Н (Заявитель) '.$familya.' '.$imya.' '.$otchestvo.'', '', '').'  
...
elseif ($get_mode[0] == "delete" || $get_mode[0] == "confirmdelete" || $get_mode[0] == "massedit" || $get_mode[0] == "print" || $get_mode[0] == "upload")   	
	{
		$id = $_POST['dopolnitelno']; 
//		$abc = new 		HelpFFT 	($userlogin, 	$get_mode[1],  	$id, 	$mode); 
	}
