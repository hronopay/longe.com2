...
//--------------------------------------------------------------------//
	...
	if ( strstr( $DoNow, "__ooo"))				$And = " AND vidimost_yn ='да' ";
	else 										$And = "";
	if ( strstr( $DoNow, "__ooopersons"))		$And = "";

	 
	/* echo  "ДЛЯ КНОПОК -> SELECT * FROM ".$this->userlogin."_".$this->TablePostFix." WHERE idlevel_".$fieldsetnumber." = ".$this->idlevel['idlevel_'.$fieldsetnumber].$And." ORDER BY ".$vybor." ASC, id ASC <br>";      */
	


	$result = sql_do("SELECT * FROM ".$DoNow." WHERE idlevel_".$fieldsetnumber." = ".$this->idlevel['idlevel_'.$fieldsetnumber].$And." ORDER BY ".$vybor." ASC, id ASC");
...
if ($totalRows_result>$columns*2) { echo "<a  href=\"javascript:inver_mod('" . ($divInd="div".rand(1, 1000)) . "')\">Прочие</a>"; }
?>
<?php /* <a  href="javascript:inver_mod('<? $divInd="div".rand(1, 1000); echo $divInd;?>')">Прочие</a> */?>
<div ID="<? echo $divInd;?>"   <? if ($totalRows_result>$columns*2) { ?>style="display:none;" <? }?> >
<table border="0">
 <tr><td>
<?
	...
	while ($ro = mysql_fetch_array($result, MYSQL_BOTH)){
		...
		$id=$ro['id'];
		$napominanie = /* $ro['naimenovanie']." ". */$ro['notes'];
...
//--------------------------------------------------------------------//   
	...
	$TextKnopki =  mb_substr($ro['naimenovanie'],0,10,'utf-8')  ; /* $napominanie = $ro['telefon']; */
	}
	elseif 	(strstr($DoNow,"prices"))	{	  //
	$TextKnopki =  "с ". mb_substr($ro['from_date'],0,7,'utf-8') ;
	}
	...
	$TextKnopki = mb_substr($ro['nazvanie'], 0, 20,'utf-8')   ;
	}
//--------------------------------------------------------------------//   
			...
			if( ($ii%$columns) ) {?></td><td><? }
			if( !($ii%$columns) ) {			
				if ($totalRows_result-$j>$columns*2){ ?></td></tr><tr><td><? 	}
				else { ?></td></tr></table></div><table border="0"><tr><td><? 	}
				}
			$ii++;
			$j++;
	}
...
//------------   Вторяа кнопка для оквэда, Приделана временно для одновременного тестирования старого и нового оквэда
		$this->Uni_Button($fieldsetnumber+1, "", $skript."?mode=massedit2014_".$buttonMode, "post", "create2014_".$buttonMode,"Создать 2014", 0, $fieldsetnumber);
		}
//------------
