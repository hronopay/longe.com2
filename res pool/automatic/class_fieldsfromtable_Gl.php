...
class FFT_2 {
  ...
  //----------- Constructor -------------------
	function FFT_2($userlogin, $TablePostFix, $id, $get_mode, $idlevel_1, $idlevel_2) {
...
// ------------------------    Вытаскиваем адрес дома и квартиры для установки в 4-е поле жильца
	$qadr = "SELECT * FROM ".( str_replace("giltsy", "orgforma",    $this->table)     )." WHERE id = ".$this->idlevel_1;
	...
	$qadr = "SELECT * FROM ".( str_replace("giltsy", "persons",    $this->table)     )." WHERE id = ".$this->idlevel_2;
  	$qadr_result = mysql_query($qadr) or die ("<p><b>ERROR!!!</b></p><br>$php_errormsg ".mysql_errno().": ".mysql_error()."<BR>".$qadr."<BR>");
...
// ------------------------   Вытаскиваем адрес дома и квартиры для установки в 4-е поле жильца              END
	...
	do {    
		...
		$Q = $Q."`".$row_q1_result['Field']."`, ";
		 ...
		 $valuesempty = "(".$valuesempty.")";
		  $valuesempty = str_replace(   "('', '', '', ''" ,  "(NULL, '"   .$this->idlevel_1.   "', '"  .$this->idlevel_2.   "', '".$adress."'"       , $valuesempty  );
...
function PutFieldsFromTable ( )  
 ...
 ?><table border="0" width="100%">
  ...
  if(!$this->RowNumber) echo "Создание нового жильца<br>";
  else 					echo "Редактирование жильца<br>";
	?></td><td><?php 
	if($this->RowNumber) $this->Lg_Button($skript."?mode=lgotygiltsa", 'post', 'lgotygiltsa', 'Льготы Жильца', -1 /* id льготы (все) */, $this->idlevel_1/* id дома */, $this->idlevel_2/* id кварт */, $this->RowNumber /* id жильца. */ );
...
function Lg_Button($action, $method, $name, $TextKnopki, $dopolnitelno/* для RowNumber */, $idlevel_1, $idlevel_2 , $idlevel_3){
?>
<form action="<?php echo $action ?>" method="<?php echo $method ?>" enctype="application/x-www-form-urlencoded" name="<?php echo $name ?>">
