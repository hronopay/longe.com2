...
include('class2admin.php'); 
include('class4admin.php'); 
session_start();

include ('include/functions.php');
$FileName = "make_db_by_fotos_withkeywords.php";
$FileHeader = "Создание БД для галерей";
$FileHeader1 = "";
$FileSlogan = "";
$RightColumn = "Ваша Корзина (<a href=\"buying_order.php\">изменить</a>)";
...
if (!isset($_SESSION['fnmbr'])) $_SESSION['fnmbr']=1;

if ( !isset($_POST['spisok_name']) || $_POST['spisok_name'] == '') ChooseList ();
elseif ($_POST['spisok_name']  == 'Давай!!!') {
	echo 'Wait!!!<br>'; 
	GrabURL_links ($_POST['url'], $_POST['new_spisok'].'.txt');
	}
else {
	makeFotoDir($_POST['spisok_name']);
	GrabByList ( $_POST['spisok_name'] , $_POST['fotonumber']);
	}
...
#-------------------------------------------------------------------------
	...
	if ($width_orig > $height_orig) return (false); 
#------------------ Адрес фотографии -------------------------------------
